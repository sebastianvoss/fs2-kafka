...
final class ConsumerRecordSpec extends BaseSpec {
  describe("ConsumerRecord#fromJava") {
    it("should convert timestamps") {
      ...
      check(NO_TIMESTAMP, NO_TIMESTAMP_TYPE)(_.timestamp shouldBe None)
      check(NO_TIMESTAMP, CREATE_TIME)(_.timestamp shouldBe None)
      check(NO_TIMESTAMP, LOG_APPEND_TIME)(_.timestamp shouldBe None)

      check(0, NO_TIMESTAMP_TYPE)(_.timestamp shouldBe None)
      check(0, CREATE_TIME)(_.timestamp.flatMap(_.createTime) shouldBe Some(0))
      check(0, LOG_APPEND_TIME)(_.timestamp.flatMap(_.logAppendTime) shouldBe Some(0))
    }
